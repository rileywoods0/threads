...
Write-Host "`n[2/5] Start session"
...
Write-Host "`n[4/5] End session (generate snapshot)"
$endBody = @{ session_id = $sessionId } | ConvertTo-Json
$snapshot = Invoke-RestMethod -Uri "$BackendUrl/session/end" -Method Post -ContentType "application/json" -Body $endBody
$snapshotId = $snapshot.id
Write-Host "SnapshotId: $snapshotId"

Write-Host "`n[5/5] List snapshots + fetch by id"
